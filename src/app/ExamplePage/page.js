...
function TopBar() {
  return (
    <div className="w-full h-10 bg-slate-300 shadow-xl z-10">
      This is the TopBar
    </div>
  )
}
  ...
  return (
    <div className="flex flex-col w-60 h-full gap-1 bg-slate-200 z-5">
      ...
      {listItems?.map((item)=> {
        ...
        <p>{item.type} {item.completed ? (<div className="text-green-600">complete</div>) : (<div className="text-yellow-600">in progress</div>)}</p>
        </div>
      );})}
    </div>
  );
